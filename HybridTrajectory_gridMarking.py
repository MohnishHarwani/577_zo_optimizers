...
class GradReliability:
    ...
    def __init__(self, dim, k=32, beta_mean=0.9, beta_var=0.98, device='cpu', seed=0):
        ...
        self.v = torch.zeros(k, device=device)  # EMA of squared deviation (variance proxy)
    ...
    @torch.no_grad()
    def update_and_score(self, g_vec: torch.Tensor, mom_vec: torch.Tensor | None = None):
        """
        ...
        if self.prev_g is not None:
            cos_prev = torch.dot(g_vec, self.prev_g) / (g_vec.norm() * self.prev_g.norm() + self.eps)
        self.prev_g = g_vec.detach().clone()

        cos_mom = None
        if mom_vec is not None and mom_vec.norm() > 0:
            cos_mom = torch.dot(g_vec, mom_vec) / (g_vec.norm() * mom_vec.norm() + self.eps)

        # Map cos in [-1,1] to [0,1]
        def cos01(x): return 0.5 * (x + 1.0)
...
@torch.no_grad()
def two_point_zo_step(model, loss_fn, data_iter, mask, sigma, lr, dirs=16, device='cpu'):
    """
    ...
    for j in range(dirs):
        ...
        loss_pos = loss_fn(logits.transpose(0, 1).reshape(-1, logits.size(-1)), y.view(-1)).detach()
        # f(w - sigma d)
        set_params_from_vector_(model, w - sigma * d)
        logits = model(x.transpose(0, 1), src_mask=mask)
        loss_neg = loss_fn(logits.transpose(0, 1).reshape(-1, logits.size(-1)), y.view(-1)).detach()
...
def main():
    ...
    # Field options
    ap.add_argument('--field_grid', type=int, default=41, help='odd number, grid per axis')
    ap.add_argument('--field_radius', type=float, default=2.5, help='box half-width in units of param std at init')
    ap.add_argument('--field_batches', type=int, default=2, help='avg these many minibatches per grid point')
    ap.add_argument('--viz_batch_size', type=int, default=8, help='batch size during field eval')
    ap.add_argument('--eval_seq_len', type=int, default=128, help='shorter seq len to speed up field eval')

    # Heatmap scaling
    ap.add_argument('--mag_scale', choices=['linear', 'log'], default='log')
    ap.add_argument('--mag_clip_p', type=float, default=99.5, help='percentile clip for grad magnitude')

    # Trajectory capture/plotting
    ap.add_argument('--traj_every', type=int, default=5, help='record param vector every N training steps')
    ap.add_argument('--traj_width', type=float, default=2.4, help='trajectory line width')
    ap.add_argument('--traj_alpha', type=float, default=0.95, help='trajectory line alpha [0,1]')
    ap.add_argument('--traj_markersize', type=float, default=9.0, help='start/end marker size')

    # Plane & center choices
    ap.add_argument('--plane', choices=['random', 'traj_pca', 'enddir'], default='traj_pca')
    ap.add_argument('--center_mode', choices=['init', 'trained', 'traj_mean'], default='trained')

    # Hybrid training knobs
    ap.add_argument('--reliability_k', type=int, default=32, help='# random projections for SNR')
    ap.add_argument('--R_threshold', type=float, default=0.45, help='below this, use ZO step')
    ap.add_argument('--zo_dirs', type=int, default=4, help='# directions for two-point ZO')
    ap.add_argument('--zo_sigma', type=float, default=1e-3, help='perturbation scale for ZO')
    ap.add_argument('--zo_lr', type=float, default=None, help='ZO step size (defaults to --lr if None)')
    ...
    eval_dl = DataLoader(eval_ds, batch_size=args.viz_batch_size, shuffle=True, drop_last=True)
    ...
    # MODIFIED: record R and components for ZO likelihood analysis
    R_records = [] # list of (parameter_vector, R) along hybrid trajectory
    ...
    # Reliability tracker
    dim = w_init.numel()
    Rtracker = GradReliability(dim=dim, k=args.reliability_k, device=device, seed=args.seed+2025)

    pbar = tqdm(total=args.epochs * args.steps_per_epoch, desc='Training (hybrid + regular shadow)')
    for epoch in range(args.epochs):
        ...
        for i in range(args.steps_per_epoch):
            ...
            model_fo.train()
            x_fo = x.to(device); y_fo = y.to(device)
            src_fo = x_fo.transpose(0, 1)
            optimizer_fo.zero_grad(set_to_none=True)
            logits_fo = model_fo(src_fo, src_mask=mask_train)
            loss_fo = criterion(logits_fo.transpose(0,1).reshape(-1, logits_fo.size(-1)), y_fo.view(-1))
            loss_fo.backward()
            ...
            model.train()
            x_h = x.to(device); y_h = y.to(device)
            ...
            loss = criterion(logits.transpose(0,1).reshape(-1, logits.size(-1)), y_h.view(-1))
            loss.backward()
            ...
            if g_flat is not None:
                ...
                if args.max_grad_elements and arr.size > args.max_grad_elements:
                    idx = rng.choice(arr.size, size=args.max_grad_elements, replace=False)
                    arr = arr[idx]
            ...
            # MODIFIED: Record R and components along the hybrid trajectory for ZO likelihood analysis
            w_current = flatten_params(model).detach().cpu().numpy()
            R_records.append((w_current, R))
            ...
            if use_zo:
                ...
                two_point_zo_step(model, criterion, eval_iter_local, mask_eval,
                                  sigma=args.zo_sigma, lr=(args.zo_lr or args.lr),
                                  dirs=args.zo_dirs, device=device)
            ...
            # Record trajectories every N steps
            ...
            pbar.update(1)
            if steps_done % 20 == 0:
                pbar.set_postfix({'loss': f'{loss.item():.4f}', 'R': f'{R:.3f}', 'zo?': str(use_zo)})
    pbar.close()
    ...
    if len(grad_records) > 0 and PCA is not None:
        ...
        sc = plt.scatter(X2[:,0], X2[:,1], c=np.arange(X2.shape[0]), cmap='viridis', s=24)
        plt.colorbar(sc, label='training step')
        plt.title('Gradient vectors projected to 2D (PCA)')
        plt.tight_layout()
        plt.savefig(os.path.join(args.save_dir, f'grad_projection_{run_id}_hybrid_traj.png'), dpi=220)
    ...
    if args.center_mode == 'init':
        ...
        center_vec = torch.stack([t.to(w_init.device) for t in traj_params_hybrid], dim=0).mean(dim=0)
    ...
    def safe_plane_from_traj(traj_list, center_vec, seed):
        ...
        T = T[finite_rows]
        if T.shape[0] < 2:
            return random_orthonormal_directions(center_vec.numel(), device=dev, seed=seed)
    ...
    if args.plane == 'traj_pca':
        ...
        u1, u2 = safe_plane_from_traj(traj_params_hybrid, center_vec, seed=args.seed+123)
    elif args.plane == 'enddir':
        w_last = traj_params_hybrid[-1].to(center_vec.device)
        d = w_last - center_vec
        if d.norm() < 1e-12:
            u1, u2 = random_orthonormal_directions(dim, device=center_vec.device, seed=args.seed+123)
        else:
            u1 = d / (d.norm() + 1e-12)
            r = torch.randn_like(u1)
            u2 = r - (r @ u1) * u1
    ...
    lin = torch.linspace(-radius, radius, steps=args.field_grid, device=center_vec.device)
    ...
    def eval_loss_and_grad_at(w_vec):
        ...
        for (x, y) in eval_dl:
            ...
            if g is not None:
                ...
                batches += 1
            model.zero_grad(set_to_none=True)
            if batches >= args.field_batches: break
        if batches == 0:
    ...
    for i in range(args.field_grid):
        for j in range(args.field_grid):
            print(f"{field_grid_idx} / {args.field_grid ** 2}")
            field_grid_idx+=1
    ...
    # ========== MODIFIED: 2D Grid System for ZO Likelihood Analysis ==========
    # Initialize grids to track R values in 2D parameter space
    grid_size = args.field_grid
    grid_R_values = np.full((grid_size, grid_size), 1.0, dtype=np.float32)  # R values for each grid cell
    
    # Create a temporary reliability tracker for grid evaluation
    temp_Rtracker = GradReliability(dim=dim, k=args.reliability_k, device=device, seed=args.seed+2025)
    
    def to_grid_idx(x, xmin, xmax, n):
        """Convert coordinate to grid index"""
        t = (x - xmin) / (xmax - xmin + 1e-12)
        return np.clip((t * (n - 1)).astype(int), 0, n - 1)
    
    # Evaluate R at ALL grid points (not just trajectory points)
    print(f"Computing gradient reliability (R) across entire {grid_size}x{grid_size} grid...")
    for i in range(grid_size):
        for j in range(grid_size):
            # Get the parameter vector at this grid point
            a_val, b_val = A[i, j].item(), B[i, j].item()
            w_grid = center_vec + a_val*u1 + b_val*u2
            
            # Evaluate gradient and R at this point
            torch.set_grad_enabled(True)
            loss_ij, g = eval_loss_and_grad_at(w_grid)
            torch.set_grad_enabled(False)
            
            # Compute R using the gradient
            if g is not None:
                R_val, _, _, _ = temp_Rtracker.update_and_score(g.to(device), None)
                grid_R_values[i, j] = R_val
            else:
                grid_R_values[i, j] = 1.0  # Default to high reliability if no gradient
    
    # Create ZO likelihood mask based on R threshold across entire grid
    # Areas where R < threshold are likely to trigger ZO switches
    zo_likelihood_mask = (grid_R_values < args.R_threshold).astype(np.float32)
    
    # Smooth the mask for better visualization
    if np.any(zo_likelihood_mask):
        zo_likelihood_mask = gaussian_filter(zo_likelihood_mask, sigma=1.0)
    
    print(f"Found {np.sum(zo_likelihood_mask > 0.5)} grid cells with likely ZO switches (R < {args.R_threshold})")
    # ========== END NEW: 2D Grid System for ZO Likelihood Analysis ==========
    ...
    if (traj_h.size > 0) or (traj_r.size > 0) or (zo_pts.size > 0):
        tx = np.concatenate([traj_h[:,0]] + ([traj_r[:,0]] if traj_r.size else []) + ([zo_pts[:,0]] if zo_pts.size else []))
        ty = np.concatenate([traj_h[:,1]] + ([traj_r[:,1]] if traj_r.size else []) + ([zo_pts[:,1]] if zo_pts.size else []))
    ...
    def draw_both_trajs_and_zo(ax):
        ...
        # Hybrid
        line_h, = ax.plot(traj_h[:,0], traj_h[:,1], color='white',
                          linewidth=args.traj_width, alpha=args.traj_alpha, zorder=6, label='Hybrid')
        line_h.set_path_effects([patheffects.Stroke(linewidth=args.traj_width+1.6,
                                                    foreground='black', alpha=0.75),
                                 patheffects.Normal()])
        ax.scatter(traj_h[0,0], traj_h[0,1], s=args.traj_markersize**2,
                   c='#33dd33', edgecolors='black', linewidths=0.9, zorder=7)
        ax.scatter(traj_h[-1,0], traj_h[-1,1], s=args.traj_markersize**2,
                   c='#ff3333', edgecolors='black', linewidths=0.9, zorder=7)
        # Regular
        if traj_r.size:
            line_r, = ax.plot(traj_r[:,0], traj_r[:,1], color='#66ddff',
                              linewidth=args.traj_width*0.9, alpha=0.85, zorder=5, label='Regular (FO-only)')
            line_r.set_path_effects([patheffects.Stroke(linewidth=args.traj_width*0.9+1.4,
                                                        foreground='black', alpha=0.55),
                                     patheffects.Normal()])
        # ZO switch markers (orange)
        if zo_pts.size:
            ax.scatter(zo_pts[:,0], zo_pts[:,1], s=42, marker='x', c='#ffa500',
                       edgecolors='black', linewidths=0.7, zorder=8, label='ZO step')
            # Highlight first switch (star, bigger)
            ax.scatter(zo_pts[0,0], zo_pts[0,1], s=120, marker='*', c='#ffa500',
                       edgecolors='black', linewidths=0.8, zorder=9, label='First ZO switch')
        ax.legend(loc='upper right', frameon=True)

    # 1) MODIFIED: Gradient magnitude heatmap + trajectories + ZO marker + ZO likelihood overlay
    fig, ax = plt.subplots(figsize=(7,7))
    im = ax.imshow(mag_vis, origin='lower', extent=extent, aspect='equal', cmap='magma')
    cbar = fig.colorbar(im); cbar.set_label(mag_label)

    # ========== NEW: Overlay ZO likelihood mask (areas where R < threshold, likely to trigger ZO switches) ==========
    if np.any(zo_likelihood_mask > 0):
        ax.imshow(zo_likelihood_mask, origin='lower', extent=extent, cmap='Reds',
                  alpha=0.4, zorder=3)
        # Add contour line for clarity
        ax.contour(a, b, zo_likelihood_mask, levels=[0.5], colors='darkred', 
                  linewidths=2, alpha=0.8, zorder=4)
    # ========== END NEW: ZO likelihood mask overlay ==========

    draw_both_trajs_and_zo(ax)
    ax.set_title(f'Gradient Magnitude + ZO Likelihood Regions  (plane: {args.plane}, center: {args.center_mode})')
    ax.set_xlabel('a (along u1)'); ax.set_ylabel('b (along u2)')
    
    # ========== NEW: Add legend for ZO likelihood ==========
    if np.any(zo_likelihood_mask > 0):
        ax.scatter([], [], c='red', alpha=0.4, s=50, label=f'Likely ZO regions (R < {args.R_threshold})')
        ax.legend(loc='upper right', frameon=True)
    # ========== END NEW: ZO likelihood legend ==========
    
    fig.tight_layout()
    fig.savefig(os.path.join(args.save_dir, f'grad_mag_heatmap_{run_id}_hybrid_traj.png'), dpi=220)
    ...
    draw_both_trajs_and_zo(ax)
    ax.set_title(f'Gradient Direction (hue) + Magnitude (value) + Trajectories\n(plane: {args.plane}, center: {args.center_mode})')
    ax.set_xlabel('a (along u1)'); ax.set_ylabel('b (along u2)')
    fig.tight_layout()
    fig.savefig(os.path.join(args.save_dir, f'grad_angle_hsv_{run_id}_hybrid_traj.png'), dpi=220)
    ...
    draw_both_trajs_and_zo(ax)
    ax.set_title(f'Loss Landscape + Trajectories  (plane: {args.plane}, center: {args.center_mode})')
    ax.set_xlabel('a (along u1)'); ax.set_ylabel('b (along u2)')
    fig.tight_layout()
    fig.savefig(os.path.join(args.save_dir, f'loss_heatmap_{run_id}_hybrid_traj.png'), dpi=220)


    # Helpful console printout
    if zo_event_steps:
        print(f'ZO steps occurred at global step indices (post-update): {zo_event_steps}')
        print(f'First ZO switch at step {zo_event_steps[0]}')
    else:
        print('No ZO steps occurred (R never fell below threshold).')

    # ========== NEW: Print ZO likelihood analysis summary ==========
    if np.any(zo_likelihood_mask > 0):
        total_cells = grid_size * grid_size
        likely_cells = np.sum(zo_likelihood_mask > 0.5)
        coverage = (likely_cells / total_cells) * 100
        min_R = np.min(grid_R_values)
        max_R = np.max(grid_R_values)
        print(f'ZO Likelihood Analysis: {likely_cells}/{total_cells} grid cells ({coverage:.1f}%) show likely ZO switch regions')
        print(f'R values across entire grid: min={min_R:.3f}, max={max_R:.3f}')
        print(f'R threshold used: {args.R_threshold}')
    else:
        print('No ZO likelihood regions found (all R values above threshold)')
    # ========== END NEW: ZO likelihood analysis summary ==========
